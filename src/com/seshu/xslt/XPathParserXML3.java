...
public class XPathParserXML3 {
	...
	public static void main(String[] args) {
		...
		try {
			fstream = new FileInputStream("C:\\Projects\\Seshu\\ReadAA\\input\\PLC15-18-New\\PLC15-18-New.txt");
			in = new DataInputStream(fstream);
			br = new BufferedReader(new InputStreamReader(in));
			String line = br.readLine();
			lineSet.add(line);
			String tag = "<PLC2socketListener>";
			String content = tag;
			nodeSet.add(content);
			int key = 1;
			endTagLevels.put(key++, "PLC2socketListener");
			do {				
				if (line.contains("/")) {
				...
				line = br.readLine();
				if(line == null && endTagLevels.size() > 0) {
			...
			FileWriter writer = new FileWriter("C:\\Projects\\Seshu\\ReadAA\\sample\\XMLFromXPaths.xml");
